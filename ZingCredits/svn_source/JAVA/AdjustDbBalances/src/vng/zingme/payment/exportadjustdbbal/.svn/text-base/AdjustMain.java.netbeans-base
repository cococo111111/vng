 ...
 */
package vng.zingme.payment.exportadjustdbbal;
...
public class AdjustMain {
    ...
    public static void main(String[] args) {
        ...
        ConfigUtil.loadConfigFile(config + "/exportzingcreditsdataadjustdbbal.config", "appActions");


        //active config
        Config.timeOut = 1001;
        exportAdjustBalances();
    }
    ...
    private static void exportAdjustBalances() {
                    ...
                    while (hope_count > 0 && !ss) {
                        ...
                        try {
                            ...
                            if (Math.abs(balance.currentBalance - balance1) > esilon) {
                                /*if (adjustdbBalace(integer, balance.currentBalance) >= 0) {
                                    output_diff.write(integer + "\t" + balance.currentBalance + "\t" + balance1 + "\t" + (balance.currentBalance - balance1) + "\n");
                                    ss = true;
                                }*/
                                output_diff.write(integer + "\t" + balance.currentBalance + "\t" + balance1 + "\t" + (balance.currentBalance - balance1) + "\n");
                                output_diff.write(buildSQLQuery(integer, balance.currentBalance, balance1) + "\n");
                            }
                            ss = true;
    ...
    private static int adjustdbBalace(int userID, double balance) {
        int res = -1;
        Connection conn = null;
        try {
            conn = DBConnectionManager.getInstance().getConnection(1000);
        ...
        PreparedStatement stmt = null;
        try {
            stmt = conn.prepareStatement("update zingcredits.accounts set amount=? where userid=?");
    ...
    private static String buildSQLQuery(int userID, double cacheBal, double dbBal) {
        String result = "update zingcredits.accounts set amount=" + cacheBal + " where userid=" + userID + " and amount=" + dbBal;
        return result;
    }
}
