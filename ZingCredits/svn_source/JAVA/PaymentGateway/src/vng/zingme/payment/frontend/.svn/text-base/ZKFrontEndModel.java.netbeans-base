package vng.zingme.payment.frontend;
 ...
 * @author root
 */
public class ZKFrontEndModel {

	private ZKFrontEndModel() {
		try {
	...
	public static ZKFrontEndModel getInstance() {
		if (_instance == null) {
			synchronized (lockObj) {
				if (_instance == null) {
					_instance = new ZKFrontEndModel();
				}
			}
		}
		return _instance;
	...
	private static ZKFrontEndModel _instance = null;
	...
	public int storepushTransaction(T_Transaction transaction, String auth) {
		...
		try {
			ZKFrontEndModel._zk.create(tranxPath, _bpushdata, ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
			if (_eventmode != 0) {
				try {
