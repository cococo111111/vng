...
include_once $GLOBALS['SCRIBE_ROOT'].'/zscribe.php';
...
class ZScriber {
	...
	$this->_socket = new TSocketPool($this->_scribe_servers, $this->_scribe_ports );
	...
	// Create the client
	$this->_scribe_client = new zscribeClient($this->_protocol);
	...
	try {
	    ...
	    $arr_msg[]= $msg;
	    $this->openTransport();
	    return $result = $this->_scribe_client->Log($arr_msg);
	...
	public function writeLogs($category,$messages) {
		try {
    ...
    public function writeLog2($category,$message) {
	try {
	    $arr_msg = array();
	    $msg = new LogEntry;
	    $msg->category = $category;
	    ...
	    $arr_msg[]= $msg;
	    $this->openTransport();
	    $this->_scribe_client->Log2($arr_msg);
	    return true;
	}
	catch(Exception $ex) {
	    $this->closeTransport();
	...
	public function writeLogs2($category,$messages) {
		try {
