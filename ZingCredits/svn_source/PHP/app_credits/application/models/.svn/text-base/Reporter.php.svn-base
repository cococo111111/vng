...
$groupTransactions[] =$a;
	...
	public function getRemainBalance($fromDate,$toDate){
		try{
			
			$query='select * from remain_balance where analyticDate>=? and analyticDate<=?';
			$result = $this->db->fetchAll($query,array($fromDate,$toDate));
		}  catch (Exception $e){            
		    throw $e;
		}
		return $result;
