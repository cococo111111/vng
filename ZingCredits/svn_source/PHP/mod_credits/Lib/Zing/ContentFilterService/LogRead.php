...
class LogRead {
	...
	public static function getInstance($options) {
		...
		if (!isset ($instance[$key])) {
			$instance[$key] = new FollowerRead($options);
		} // if
		return $instance[$key];
	} // getInstance
	public function LogRead($options) {
		if (!is_array($options)) {
			throw new Exception('Error found in configuration !!!');
		}
		if (array_key_exists('host', $options)) {
		...
		$this->client = new LogCommentFilterReadServiceClient($this->protocol);
	}
	...
	public function  getLog($log, $fromTime, $toTime, $vips){
		try {
			$this->openTransport();
			$data = $this->client->getLog($log, $fromTime, $toTime, $vips);
		} catch (Exception $e) {
			echo $e;
			$this->closeTransport();
			return array ();
