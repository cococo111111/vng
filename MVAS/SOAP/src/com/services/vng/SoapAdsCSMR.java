...
public class SoapAdsCSMR extends ContentAbstract {
	...
	private static final int CHARGE_MONEY = 1;
	...
	private static final String MESSAGE_REGISTER = "Cam on ban da dang ky khong nhan thong bao. Neu can ho tro xin goi so 1900561558. Cam on.";
	
	private static final String className = "com.services.vng.SoapAdsCSMR";
	//ADSR = CSM ADS: for Real
	private static final String CSM_CP_CODE = "ADCSM";
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword) throws Exception {
		...
		WSConfig wsConfig = WSConfigLoader.getInstance().getWSConfigByCpCode(CSM_CP_CODE);
		if (wsConfig == null){	
			Util.logger.warn(this.getClass().getName()+ ".getMessages():  URL WebService for CPCODE:["+ CSM_CP_CODE +"] Not Found");
			msgObject.setUsertext(Constants._prop.getProperty("MESSAGE_SYSTEM_OVERLOAD"));
			msgObject.setMsgtype(CHARGE_BACK);
			messages.add(new MsgObject(msgObject));
			return messages;
		...
		while (iRetries > 0) {
			try {
				...
				if ("202".equals(result)) {
					...
					if (add2CanceledAds(msgObject.getUserid())){						
						msgObject.setUsertext(MESSAGE_REGISTER);
						msgObject.setMsgtype(CHARGE_MONEY);
						messages.add(new MsgObject(msgObject));
						return messages;
					}else{
						msgObject.setUsertext(Constants._prop.getProperty("MESSAGE_SYSTEM_OVERLOAD"));
					...
					writeLogError(msgObject, result, "Bad Request", iRetries);					
					msgObject.setMsgNotes("Respone Code: [" + result +"]. Bad Request");
					add2SMSSendFailed(msgObject);
					msgObject.setUsertext(Constants._prop.getProperty("MESSAGE_SYSTEM_OVERLOAD"));
					msgObject.setMsgtype(CHARGE_BACK);
					messages.add(new MsgObject(msgObject));
					return messages;
				}else if ("405".equals(result) || "409".equals(result)) {
