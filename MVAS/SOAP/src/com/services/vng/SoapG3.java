...
public class SoapG3 extends ContentAbstract {
	private static final String className = "com.services.vng.SoapG3";
	private static final String TUTHIENVNG_CP_CODE = "SG3";         
	
	private static final String BUSY_SYSTEM = "He thong hien tai dang qua tai. Ban hay thu lai. DT ho tro 1900555584";
	private static final int CHARGE_BACK = 2;
	private static final String INVALID_SYNTAX = "Tin nhan cua ban khong dung cu phap, xin hay kiem tra va thu lai. Neu can ho tro xin goi so 1900555584. Cam on.";
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword) throws Exception {
		...
		String infotemp = msgObject.getUsertext();
		if(infotemp.startsWith("888_") || infotemp.startsWith("88_") ||infotemp.startsWith("8888_"))
		{
			infotemp = infotemp.replace("_", " ");
		}
		if (!msgObject.getKeyword().equalsIgnoreCase(infotemp.split(" ")[0].trim()) ){
		...
		while (iRetries > 0) {
			...
			try {
				...
				Util.logger.info("TimeSendTo888Service:"+ timesend);
				// 202 transaction OK!
				if ("1".equals(result)) {
