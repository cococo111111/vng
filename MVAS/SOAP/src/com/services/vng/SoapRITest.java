...
public class SoapRITest extends ContentAbstract {
	private static final String className = "com.services.vng.SoapRITest";
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword) throws Exception {
		...
		if (messageInfor.get(ZMUtils.RINGTONG_ID).equals("") || !SoapUtils.isDigits(messageInfor.get(ZMUtils.RINGTONG_ID))){
			msgObject.setUsertext("[6269] Ban da nhap sai cu phap. Hay kiem tra san pham ban muon tai ve. DT ho tro 1900561558 hay vao http://hotro.zing.vn");
			msgObject.setMsgtype(1);
			messages.add(new MsgObject(msgObject));
			return messages;
		}
		
		Map<String, String> cpInfor = ZMUtils.getCPCodeFromZingMobile(messageInfor.get(ZMUtils.RINGTONG_ID), ZMUtils.TABLE_RINGTONG);
		Util.logger.info("cpInfor:" + cpInfor);
		
		if (cpInfor.get(ZMUtils.CP_CODE).equals("")){	//NOT FOUND CP_CODE
			Util.logger.warn(this.getClass().getName()+ ".getMessages(): RINGTONG_ID: [" + messageInfor.get(ZMUtils.RINGTONG_ID) + "] NOT FOUND CP_CODE");
			msgObject.setUsertext("[mKool] Nhac chuong " + messageInfor.get(ZMUtils.RINGTONG_ID) + " khong ton tai hoac da bi xoa. Ban hay kiem tra lai tren website http://mkool.zing.vn. DT ho tro 1900561558");
			msgObject.setMsgtype(1);
			messages.add(new MsgObject(msgObject));
			return messages;
		}
		...
		while (iRetries > 0) {
			try {
				result = sendMessageMO(msgObject, wsConfig, messageInfor.get(ZMUtils.RINGTONG_ID));
				// 1 transaction OK!
				if ("1".equals(result)) {
