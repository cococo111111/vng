...
public class SoapG6 extends ContentAbstract {
	private static final String className = "com.services.vng.SoapG6";
	private static final String MUA123_CP_CODE = "SG6";
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try
		{
			...
			if(moneycheck == 0)
			{
				writeLogMOInfo(msgObject, timesendtoCP, "system overload");
				//mologger.close();
		...
		while (iRetries > 0) {
			...
			try {
				...
				Util.logger.info("TimeSendToG6Service:" + timesend);
				...
				if ("202".equals(result)) {
					...
					writeLogError(msgObject, result,
							"System Busy - Can not send MT to G6Service",
							iRetries);
					writeLogMOInfo(msgObject, timesendtoCP, result);
					//mologger.close();
					msgObject.setMsgNotes("Respone Code: [" + result
