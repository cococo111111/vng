...
public class Soap123MUAR extends ContentAbstract {
	private static final String className = "com.services.vng.Soap123MUAR";
	private static final String MUA123_CP_CODE = "MUAR";         
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword) throws Exception {
		...
		WSConfig wsConfig = WSConfigLoader.getInstance().getWSConfigByCpCode(MUA123_CP_CODE);
		if (wsConfig == null){	
			Util.logger.warn(this.getClass().getName()+ ".getMessages():  URL WebService for CPCODE:["+ MUA123_CP_CODE +"] Not Found");
		...
		while (iRetries > 0) {
			try {
				...
				if ("202".equals(result)) {
					...
					writeLogError(msgObject, result , "System Busy - Can not send MT to 123MuaServiceMT", iRetries);					
					msgObject.setMsgNotes("Respone Code: [" + result +"]. System Busy - Can not send MT to 123MuaServiceMT");
					add2SMSSendFailed(msgObject);
					msgObject.setUsertext(BUSY_SYSTEM);
					msgObject.setMsgtype(CHARGE_BACK);
					messages.add(new MsgObject(msgObject));
