...
public class Soap123SHOP extends ContentAbstract {
	private static final String className = "com.services.vng.Soap123SHOP";
	private static final String SHOP123_CP_CODE = "S123R";         
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword) throws Exception {
		...
		WSConfig wsConfig = WSConfigLoader.getInstance().getWSConfigByCpCode(SHOP123_CP_CODE);
		if (wsConfig == null){	
			Util.logger.warn(this.getClass().getName()+ ".getMessages():  URL WebService for CPCODE:["+ SHOP123_CP_CODE +"] Not Found");
		...
		while (iRetries > 0) {
			try {
				...
				if ("202".equals(result)) {
					...
					writeLogError(msgObject, result , "System Busy - Can not send MT to 123ShopServiceMT", iRetries);					
					msgObject.setMsgNotes("Respone Code: [" + result +"]. System Busy - Can not send MT to 123ShopServiceMT");
					add2SMSSendFailed(msgObject);
					msgObject.setUsertext(BUSY_SYSTEM);
					msgObject.setMsgtype(CHARGE_BACK);
					messages.add(new MsgObject(msgObject));
