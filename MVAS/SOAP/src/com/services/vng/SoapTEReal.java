...
public class SoapTEReal extends ContentAbstract {
	private static final String className = "com.services.vng.SoapTEReal";
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword) throws Exception {
		...
		if (messageInfor.get(ZMUtils.TEXT_ID).equals("") || !SoapUtils.isDigits(messageInfor.get(ZMUtils.TEXT_ID))){
		...
		Map<String, String> cpInfor = ZMUtils.getCPCodeFromZingMobile(messageInfor.get(ZMUtils.TEXT_ID), ZMUtils.TABLE_TEXT);
		Util.logger.info("cpInfor:" + cpInfor);
		
		if (cpInfor.get(ZMUtils.CP_CODE).equals("")){	//NOT FOUND CP_CODE
			Util.logger.warn(this.getClass().getName()+ ".getMessages(): TEXT_ID: [" + messageInfor.get(ZMUtils.TEXT_ID) + "] NOT FOUND CP_CODE");
			msgObject.setUsertext("[mKool] Giai Tri " + messageInfor.get(ZMUtils.TEXT_ID) + " khong ton tai hoac da bi xoa. Ban hay kiem tra lai tren website http://mkool.zing.vn. DT ho tro 1900561558");
			msgObject.setMsgtype(1);
			messages.add(new MsgObject(msgObject));
			return messages;
		}
		...
		while (iRetries > 0) {
			try {
				result = sendMessageMO(msgObject, wsConfig, messageInfor.get(ZMUtils.TEXT_ID));
				// 1 transaction OK!
				if ("1".equals(result)) {
	...
	private Map<String, String> analyseMessage(String message){
		...
		if (removedSpaceMessages.size() == 1){
			...
			messageInfor.put(ZMUtils.TEXT_ID, "");
			messageInfor.put(ZMUtils.FRIEND_MOBILE, "");
		}else if(removedSpaceMessages.size() == 2){
			messageInfor.put(ZMUtils.KEYWORD, removedSpaceMessages.get(0));
			messageInfor.put(ZMUtils.TEXT_ID, removedSpaceMessages.get(1));
			messageInfor.put(ZMUtils.FRIEND_MOBILE, "");
