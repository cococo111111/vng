...
public class SoapTHTest extends ContentAbstract {
	private static final String className = "com.services.vng.SoapTHTest";
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword) throws Exception {
		...
		if (messageInfor.get(ZMUtils.THEME_ID).equals("") || !SoapUtils.isDigits(messageInfor.get(ZMUtils.THEME_ID))){
			msgObject.setUsertext("[6369] Ban da nhap sai cu phap. Hay kiem tra san pham ban muon tai ve. DT ho tro 1900561558 hay vao http://hotro.zing.vn");
			msgObject.setMsgtype(1);
			messages.add(new MsgObject(msgObject));
			return messages;
		}
		
		Map<String, String> cpInfor = ZMUtils.getCPCodeFromZingMobile(messageInfor.get(ZMUtils.THEME_ID), ZMUtils.TABLE_THEME);
		Util.logger.info("cpInfor:" + cpInfor);
		
		if (cpInfor.get(ZMUtils.CP_CODE).equals("")){	//NOT FOUND CP_CODE
			Util.logger.warn(this.getClass().getName()+ ".getMessages(): THEME_ID: [" + messageInfor.get(ZMUtils.THEME_ID) + "] NOT FOUND CP_CODE");
			msgObject.setUsertext("[mKool] Theme " + messageInfor.get(ZMUtils.THEME_ID) + " khong ton tai hoac da bi xoa. Ban hay kiem tra lai tren website http://mkool.zing.vn. DT ho tro 1900561558");
			msgObject.setMsgtype(1);
			messages.add(new MsgObject(msgObject));
			return messages;
		}
		...
		while (iRetries > 0) {
			try {
				result = sendMessageMO(msgObject, wsConfig, messageInfor.get(ZMUtils.THEME_ID));
				// 1 transaction OK!
				if ("1".equals(result)) {
	...
	private Map<String, String> analyseMessage(String message){
		...
		if (removedSpaceMessages.size() == 1){
			...
			messageInfor.put(ZMUtils.THEME_ID, "");
			messageInfor.put(ZMUtils.FRIEND_MOBILE, "");
		}else if(removedSpaceMessages.size() == 2){
			messageInfor.put(ZMUtils.KEYWORD, removedSpaceMessages.get(0));
			messageInfor.put(ZMUtils.THEME_ID, removedSpaceMessages.get(1));
			messageInfor.put(ZMUtils.FRIEND_MOBILE, "");
