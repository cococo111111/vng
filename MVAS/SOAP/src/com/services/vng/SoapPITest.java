...
public class SoapPITest extends ContentAbstract {
	private static final String className = "com.services.vng.SoapPITest";
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword) throws Exception {
		...
		if (messageInfor.get(ZMUtils.PICTURE_ID).equals("") || !SoapUtils.isDigits(messageInfor.get(ZMUtils.PICTURE_ID))){
		...
		Map<String, String> cpInfor = ZMUtils.getCPCodeFromZingMobile(messageInfor.get(ZMUtils.PICTURE_ID), ZMUtils.TABLE_PICTURE);
		Util.logger.info("cpInfor:" + cpInfor);		
		
		if (cpInfor.get(ZMUtils.CP_CODE).equals("")){	//NOT FOUND CP_CODE
			Util.logger.warn(this.getClass().getName()+ ".getMessages(): PICTURE_ID: [" + messageInfor.get(ZMUtils.PICTURE_ID) + "] NOT FOUND CP_CODE");
			msgObject.setUsertext("[mKool] Hinh nen " + messageInfor.get(ZMUtils.PICTURE_ID) + " khong ton tai hoac da bi xoa. Ban hay kiem tra lai tren website http://mkool.zing.vn. DT ho tro 1900561558");
			msgObject.setMsgtype(1);
			messages.add(new MsgObject(msgObject));
			return messages;
		}
		...
		while (iRetries > 0) {
			try {
				result = sendMessageMO(msgObject, wsConfig, messageInfor.get(ZMUtils.PICTURE_ID));
				// 1 transaction OK!
				if ("1".equals(result)) {
	...
	private Map<String, String> analyseMessage(String message){
		...
		if (removedSpaceMessages.size() == 1){
			...
			messageInfor.put(ZMUtils.PICTURE_ID, "");
			messageInfor.put(ZMUtils.FRIEND_MOBILE, "");
		}else if(removedSpaceMessages.size() == 2){
			messageInfor.put(ZMUtils.KEYWORD, removedSpaceMessages.get(0));
			messageInfor.put(ZMUtils.PICTURE_ID, removedSpaceMessages.get(1));
			messageInfor.put(ZMUtils.FRIEND_MOBILE, "");
