...
public class SoapVNGRealNew extends ContentAbstract {
	...
	private static final String className = "com.services.vng.SoapVNGRealNew";
	//SMSPN = Payment Real N: for Real
	private static final String PAYMMENT_CP_CODE = "SMPMN";
			...
			while (iRetries > 0) {
				try {
					...
					if ("202".equals(result)) {
						...
						msgObject.setMsgNotes("Respone Code: [" + result +"]. Spam message");
						add2SMSSendFailed(msgObject);
						return null;
					}else if ("406".equals(result)) {
						writeLogError(msgObject,  result,  "Invalid ServiceID or can not get exchange rate", iRetries);					
						msgObject.setMsgNotes("Respone Code: [" + result +"]. Invalid ServiceID or can not get exchange rate");
						add2SMSSendFailed(msgObject);
						...
						writeLogError(msgObject, result, "Invalid pattern", iRetries);					
						msgObject.setMsgNotes("Respone Code: [" + result +"]. Invalid pattern");
						add2SMSSendFailed(msgObject);
						return null;
					}else if ("405".equals(result)) {
						writeLogError(msgObject, result , "Can not connect Passport System. Can Not Save SMS Information", iRetries);					
						msgObject.setMsgNotes("Respone Code: [" + result +"]. Can not connect Passport System. Can Not Save SMS Information");
						add2SMSSendFailed(msgObject);
						return null;
					}else if ("204".equals(result)) {
						writeLogError(msgObject, result,"Account does not exist in Passport System", iRetries);					
						msgObject.setMsgNotes("Respone Code: [" + result +"]. Account does not exist in Passport System");
						add2SMSSendFailed(msgObject);
						return null;
					}else if ("409".equals(result)) {
						writeLogError(msgObject, result, "Banned Account", iRetries);					
						msgObject.setMsgNotes("Respone Code: [" + result +"]. Banned Account");
						add2SMSSendFailed(msgObject);
						return null;
					}else if ("410".equals(result)) {
						writeLogError(msgObject, result, "Unregister User Phone", iRetries);					
						msgObject.setMsgNotes("Respone Code: [" + result +"]. Unregister User Phone");
						add2SMSSendFailed(msgObject);
						return null;
					}else {
