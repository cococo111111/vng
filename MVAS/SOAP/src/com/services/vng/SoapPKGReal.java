...
public class SoapPKGReal extends ContentAbstract {
	private static final String className = "com.services.vng.SoapPKGReal";
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword) throws Exception {
		...
		if (messageInfor.get(ZMUtils.PACKAGE_ID).equals("") || !SoapUtils.isDigits(messageInfor.get(ZMUtils.PACKAGE_ID))){
		...
		Map<String, String> cpInfor = ZMUtils.getCPCodeFromZingMobile(Integer.parseInt(messageInfor.get(ZMUtils.PACKAGE_ID)), ZMUtils.CATEGORY_PACKAGE);
		Util.logger.info("cpInfor:" + cpInfor);		
		
		if (cpInfor.get(ZMUtils.CP_CODE).equals("")){	//NOT FOUND CP_CODE
			Util.logger.warn(this.getClass().getName()+ ".getMessages(): PACKAGE_ID: [" + messageInfor.get(ZMUtils.PACKAGE_ID) + "] NOT FOUND CP_CODE");
			msgObject.setUsertext("[mKool] BST " + messageInfor.get(ZMUtils.PACKAGE_ID) + " khong ton tai hoac da bi xoa. Ban hay kiem tra lai tren website http://mkool.zing.vn. DT ho tro 1900561558");
			msgObject.setMsgtype(1);
			messages.add(new MsgObject(msgObject));
			return messages;
		}
				
		if (cpInfor.get(ZMUtils.CP_CODE).equals("-1") ){	//System ZingSms is busy
		...
		while (iRetries > 0) {
			try {
				result = sendMessageMO(msgObject, wsConfig, messageInfor.get(ZMUtils.PACKAGE_ID));
				// 1 transaction OK!
				if ("1".equals(result)) {
	...
	private Map<String, String> analyseMessage(String message){
		...
		if (removedSpaceMessages.size() == 1){
			...
			messageInfor.put(ZMUtils.PACKAGE_ID, "");
			messageInfor.put(ZMUtils.FRIEND_MOBILE, "");
		}else if(removedSpaceMessages.size() == 2){
			messageInfor.put(ZMUtils.KEYWORD, removedSpaceMessages.get(0));
			messageInfor.put(ZMUtils.PACKAGE_ID, removedSpaceMessages.get(1));
			messageInfor.put(ZMUtils.FRIEND_MOBILE, "");
