...
public class SoapFanReal extends ContentAbstract {
	private static final String BUSY_SYSTEM = "He thong hien tai dang qua tai. Ban hay thu lai. DT ho tro 1900561558";
	
	private static final String BUSY_SYSTEM_LATER = "He thong hien tai dang qua tai. Xin ban hay thu lai sau vai phut nua. Neu can ho tro xin goi so 1900561558. Cam on.";
	
	private static final int CHARGE_BACK = 2;
	
	private static final String INVALID_SYNTAX = "[CELANO&ZING] Tin nhan cua ban khong dung cu phap. Soan Tin Fan MSBC gui 6169. Truy cap http://topvclip.mp3.zing.vn de biet them thong tin";
	
	private static final String className = "com.services.vng.SoapFanReal";
	//SMSPN = Payment Real N: for Real
	private static final String FAN_CP_CODE = "FANR";
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword) throws Exception {
		...
		WSConfig wsConfig = WSConfigLoader.getInstance().getWSConfigByCpCode(FAN_CP_CODE);
		if (wsConfig == null){	
			Util.logger.warn(this.getClass().getName()+ ".getMessages():  URL WebService for CPCODE:["+ FAN_CP_CODE +"] Not Found");
		...
		while (iRetries > 0) {
			try {
				...
				if ("202".equals(result)) {
					...
					writeLogError(msgObject, result , "Invalid ServiceID or CommandCode", iRetries);					
					msgObject.setMsgNotes("Respone Code: [" + result +"]. Invalid ServiceID or CommandCode");
					add2SMSSendFailed(msgObject);
					msgObject.setUsertext(BUSY_SYSTEM_LATER);
					msgObject.setMsgtype(CHARGE_BACK);
					messages.add(new MsgObject(msgObject));
					return messages;
				}else if ("404".equals(result)) {
					writeLogError(msgObject, result , "Username & Password does not match or IP", iRetries);					
					msgObject.setMsgNotes("Respone Code: [" + result +"]. Username & Password does not match or IP");
					add2SMSSendFailed(msgObject);
					msgObject.setUsertext(BUSY_SYSTEM_LATER);
					msgObject.setMsgtype(CHARGE_BACK);
					messages.add(new MsgObject(msgObject));
					...
					messages.add(new MsgObject(msgObject));
					return messages;
				}else if ("405".equals(result) || "406".equals(result)) {
					writeLogError(msgObject, result , "Partner Can not Save SMS Information", iRetries);					
					msgObject.setMsgNotes("Respone Code: [" + result +"]. Partner Can not Save SMS Information");
					add2SMSSendFailed(msgObject);
					msgObject.setUsertext(BUSY_SYSTEM_LATER);
					msgObject.setMsgtype(CHARGE_BACK);
					messages.add(new MsgObject(msgObject));
					...
					writeLogError(msgObject, result , "Partner send MT to MVAS failed", iRetries);					
					msgObject.setMsgNotes("Respone Code: [" + result +"]. Partner send MT to MVAS failed");
					add2SMSSendFailed(msgObject);
					msgObject.setUsertext(BUSY_SYSTEM_LATER);
					msgObject.setMsgtype(CHARGE_BACK);
					messages.add(new MsgObject(msgObject));
