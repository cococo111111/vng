...
public class SoapDABR extends ContentAbstract {
	...
	private static final String MESSAGE_SUCCESS = "[DAB-VNG] Xac nhan TK <#ZingID#>, Ten <#FullName#>, CM<#CM#> du dieu kien mo the DAB mien phi tu 15/11.";
	
	private static final String className = "com.services.vng.SoapDABR";
	private static final String MOTHE_CP_CODE = "DABR";
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword) throws Exception {
		...
		WSConfig wsConfig = WSConfigLoader.getInstance().getWSConfigByCpCode(MOTHE_CP_CODE);
		if (wsConfig == null){	
			Util.logger.warn(this.getClass().getName()+ ".getMessages():  URL WebService for CPCODE:["+ MOTHE_CP_CODE +"] Not Found");
			msgObject.setUsertext(Constants._prop.getProperty("MESSAGE_SYSTEM_OVERLOAD"));
			msgObject.setMsgtype(CHARGE_BACK);
			messages.add(new MsgObject(msgObject));
			return messages;
