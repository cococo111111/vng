...
public class SoapPayDirectBilling extends ContentAbstract {
	...
	private static final String className = "com.services.vng.SoapVNGReal";
	// SMSPN = Payment Real N: for Real
	private static final String PAY_DIRECT_BILLING_CP_CODE = "PAYDB";
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		if (keywordWhiteList != null && !inWhiteList(keywordWhiteList.getWhiteList(), msgObject.getUserid())) {
			...
			keywordWhiteList = WSConfigLoader.getInstance().getKeywordWhiteListByKeywordServiceID(
							keyword.getKeyword() + msgObject.getServiceid()
		...
		WSConfig wsConfig = WSConfigLoader.getInstance().getWSConfigByCpCode(PAY_DIRECT_BILLING_CP_CODE);
		if (wsConfig == null) {
			Util.logger.warn(this.getClass().getName()
					+ ".getMessages():  URL WebService for CPCODE:[" + PAY_DIRECT_BILLING_CP_CODE + "] Not Found");
		...
		while (iRetries > 0) {
			try {
				...
				if ("202".equals(result)) {
					...
					writeLogError(msgObject, result, "Invalid CommandCode",
							iRetries);
					...
					writeLogError(msgObject, result,
							"Username & Password does not match", iRetries);
					msgObject.setMsgNotes("Respone Code: [" + result
							+ "]. Username & Password does not match");
					add2SMSSendFailed(msgObject);
					msgObject
							.setUsertext("He thong hien tai dang qua tai. Xin ban hay thu lai sau vai phut nua. Neu can ho tro xin goi so 1900561558. Cam on.");
					msgObject.setMsgtype(CHARGE_BACK);
					messages.add(new MsgObject(msgObject));
					return messages;
				}/* else if ("4081".equals(result) || "4082".equals(result)) {
					writeLogError(msgObject, result, "Spam message", iRetries);
					msgObject.setMsgNotes("Respone Code: [" + result
							+ "]. Spam message");
					add2SMSSendFailed(msgObject);
					return null;
					...
					writeLogError(msgObject, result,
							"Invalid ServiceID or can not get exchange rate",
							iRetries);
					msgObject
							.setMsgNotes("Respone Code: ["
									+ result
									+ "]. Invalid ServiceID or can not get exchange rate");
					add2SMSSendFailed(msgObject);
					msgObject
							.setUsertext("Dich vu khong ho tro dau so nay, xin hay kiem tra va thu lai. Neu can ho tro xin goi so 1900561558. Cam on.");
					msgObject.setMsgtype(CHARGE_BACK);
					messages.add(new MsgObject(msgObject));
					return messages;
					...
					writeLogError(msgObject, result, "Invalid pattern",
							iRetries);
					msgObject.setMsgNotes("Respone Code: [" + result
							+ "]. Invalid pattern");
					add2SMSSendFailed(msgObject);
					...
					writeLogError(
							msgObject,
							result,
							"Can not connect Passport System. Can Not Save SMS Information",
							iRetries);
					msgObject
							.setMsgNotes("Respone Code: ["
									+ result
					...
					writeLogError(msgObject, result,
							"Account does not exist in Passport System",
							iRetries);
					msgObject.setMsgNotes("Respone Code: [" + result
							+ "]. Account does not exist in Passport System");
					add2SMSSendFailed(msgObject);
					...
					writeLogError(msgObject, result, "Banned Account", iRetries);
					msgObject.setMsgNotes("Respone Code: [" + result
							+ "]. Banned Account");
					add2SMSSendFailed(msgObject);
					return null;
				} /*else if ("410".equals(result)) {
					writeLogError(msgObject, result, "Unregister User Phone",
							iRetries);
					msgObject.setMsgNotes("Respone Code: [" + result
							+ "]. Unregister User Phone");
					add2SMSSendFailed(msgObject);
					return null;
				}*/ else {
					writeLogError(msgObject, result,
							"Going For Retry, Sleeping", iRetries);
					msgObject.setMsgNotes("Respone Code: [" + result
