...
public class SoapFIDOLR extends ContentAbstract {
	...
	private static final String className = "com.services.vng.SoapFIDOLR";
	//LACCR = LOCK COUNT CSTOOL: for Real
	private static final String FIDOL_CP_CODE = "FIDOR";
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword) throws Exception {
		...
		if (wsConfig == null){	
			Util.logger.warn(this.getClass().getName()+ ".getMessages():  URL WebService for CPCODE:["+ FIDOL_CP_CODE +"] Not Found");
			msgObject.setUsertext(Constants._prop.getProperty("MESSAGE_SYSTEM_OVERLOAD"));
			msgObject.setMsgtype(CHARGE_BACK);
			messages.add(new MsgObject(msgObject));
			return messages;
