...
public class SoapLockACCCSTool extends ContentAbstract {
	...
	private static final String INVALID_SYNTAX_123 = "Tin nhan cua ban khong dung cu phap, xin hay kiem tra va thu lai. Neu can ho tro xin goi so 1800585888. Cam on.";
	
	private static final String className = "com.services.vng.SoapLockACCR";
	//LACCR = LOCK COUNT CSTOOL: for Real
	private static final String LOCKACCOUNT_CP_CODE = "CSTN";
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword) throws Exception {
		...
		if (!msgObject.getKeyword().equalsIgnoreCase(msgObject.getUsertext().split(" ")[0].trim()) ){			
			if(msgObject.getUsertext().substring(1, 2).toLowerCase() == "gh" || msgObject.getUsertext().substring(1, 3).toLowerCase() == "huy"){//xu ly tin nhan fail cho 123 vn -- change so tong dai ho tro
				msgObject.setUsertext(INVALID_SYNTAX_123);
			}
			else
			{
		...
		WSConfig wsConfig = WSConfigLoader.getInstance().getWSConfigByCpCode(LOCKACCOUNT_CP_CODE);
		if (wsConfig == null){	
			Util.logger.warn(this.getClass().getName()+ ".getMessages():  URL WebService for CPCODE:["+ LOCKACCOUNT_CP_CODE +"] Not Found");
			msgObject.setUsertext(Constants._prop.getProperty("MESSAGE_SYSTEM_OVERLOAD"));
			msgObject.setMsgtype(CHARGE_BACK);
			messages.add(new MsgObject(msgObject));
			return messages;
