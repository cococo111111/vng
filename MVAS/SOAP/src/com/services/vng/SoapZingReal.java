...
public class SoapZingReal extends ContentAbstract {
	private static final String className = "com.services.vng.SoapZingReal";
	private static final String ZP_CP_CODE = "ZPR";
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword) throws Exception {
		...
		WSConfig wsConfig = WSConfigLoader.getInstance().getWSConfigByCpCode(ZP_CP_CODE);
		if (wsConfig == null){	
			Util.logger.warn(this.getClass().getName()+ ".getMessages():  URL WebService for CPCODE:["+ ZP_CP_CODE +"] Not Found");
