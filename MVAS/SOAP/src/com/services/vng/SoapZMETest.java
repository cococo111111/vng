...
public class SoapZMETest extends ContentAbstract {
	private static final String className = "com.services.vng.SoapZMETest";
	private static final String ZME_CP_CODE = "ZMET";         
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword) throws Exception {
		...
		WSConfig wsConfig = WSConfigLoader.getInstance().getWSConfigByCpCode(ZME_CP_CODE);
		if (wsConfig == null){	
			Util.logger.warn(this.getClass().getName()+ ".getMessages():  URL WebService for CPCODE:["+ ZME_CP_CODE +"] Not Found");
