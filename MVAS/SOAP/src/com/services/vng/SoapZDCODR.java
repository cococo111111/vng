...
public class SoapZDCODR extends ContentAbstract {
	...
	private static final String className = "com.services.vng.SoapZDCODR";
	//CODR = ZingDeal Code: for Real
	private static final String COD_CP_CODE = "CODR";
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword) throws Exception {
		...
		WSConfig wsConfig = WSConfigLoader.getInstance().getWSConfigByCpCode(COD_CP_CODE);
		if (wsConfig == null){	
			Util.logger.error(this.getClass().getName()+ ".getMessages():  URL WebService for CPCODE:["+ COD_CP_CODE +"] Not Found");
			msgObject.setUsertext(Constants._prop.getProperty("MESSAGE_SYSTEM_OVERLOAD"));
			msgObject.setMsgtype(CHARGE_BACK);
			messages.add(new MsgObject(msgObject));
			return messages;
