...
public class Utils {
	...
	public static int getUIDFromUName(String uName) throws ZException,
			TException {
		STDProfile2Handler profile2Handler = STDProfile2Handler
				.getMainInstance(Config2.stdProfile2Host,
						Config2.stdProfile2Port);
		OpHandle oh = new OpHandle();
		oh.setSource(Config2.stdprofile2source);
		oh.setAuth(Config2.stdprofile2auth);
	...
	public static String getAppName(String appID) throws TException {
		AppServiceHandler adminHandler = AppServiceHandler.getMainInstance(
				Utils.getConfig("gameinfor.host"),
				Integer.parseInt(Utils.getConfig("gameinfor.port")));
	...
	public static String buildAvatarPath(String username, int avatarversion,
			String size) {
		...
		return path + avatarpath + "/" + username + "_" + size + "_"
				+ avatarversion + ".jpg";
