jQuery.autocomplete = function(input, options) {
	...
	if (options.data != null) {
		...
		for ( var i = 0; i < options.data.length; i++) {
			...
			// array
			row = ((typeof options.data[i] == "string") ? [ options.data[i] ]
					: options.data[i]);
	...
	function receiveData(q, data) {
		if (data) {
			...
			if (options.autoFill
					&& ($input.val().toLowerCase() == q.toLowerCase()))
				autoFill(data[0][0]);
			showResults();
		} else {
			hideResultsNow();
	...
	function requestData(q) {
		...
		if (data) {
			...
			if (!options.makeUrl)
				var request_url = makeUrl(q);
			else
				var request_url = options.makeUrl(q);
			$.getJSON(request_url,
			// $.getJSON("/test/autocomplete/getdata",
					function(data) {
	...
	function makeUrl(q) {
		var url = options.url + "&q=cityid:1 AND pft_schoolname:"
				+ encodeURI(q) + "&cb=?";
...
jQuery.fn.autocomplete = function(url, options, data) {
	...
	options.data = ((typeof data == "object") && (data.constructor == Array)) ? data
			: null;
