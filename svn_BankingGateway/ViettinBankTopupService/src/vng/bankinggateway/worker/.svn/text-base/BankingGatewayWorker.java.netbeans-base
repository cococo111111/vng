            ...
            try {
                ...
                try {
                    tx = BankingStorageClient.getMainInstance(CommonDef.STORAGE_HOST,
                            CommonDef.STORAGE_PORT).getTransactionWithBankCode(Integer.valueOf(zionDataObj.getTransactionID()), zionDataObj.getTime(), CommonDef.BANKCODE.VIETINBANK);
                    ...
                    esaleStatus = ConfirmOrderBankServiceHandler.getInstance(CommonDef.ESALE_WEBSERVICE_URL).confirmOrderBankService(zionDataObj.getAgencyCode(),
                            tx.getRefID(), zionDataObj.getResponseCode(), zionDataObj.getBankAccountCode(), tx.getBankCode());
                } catch (Exception ex) {
