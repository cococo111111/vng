                                ...
                                for (int i = 0; i < ((Collection<?>) prop).size(); i++) {
                                        ...
                                        if (nameSystem.toLowerCase().equals("mysql")) {
                                                ...
                                                userName = config.getString("system(" + i
                                                        + ").username");
                                                dbName = config.
                                                        getString("system(" + i + ").dbName");
                                                timeOut = config.
                                                        getLong("system(" + i + ").timeOut");
                                                maxPoolSize = config.getInt(
                                                        "system(" + i + ").maxPoolSize");
                                                maxQueueSize = config.getInt(
                                                        "system(" + i + ").maxQueueSize");
                                        }
                                        if (nameSystem.toLowerCase().equals("billing")) {
                                                billingHost = config
                                                        .getString("system(" + i + ").host");
                                                billingPort = config.
                                                        getInt("system(" + i + ").port");
                                        }
                                        ...
                                        if (nameSystem.toLowerCase().equals("queue")) {
                                                ...
                                                queuePort2 = config.
                                                        getInt("system(" + i + ").port2");

                                                NumOfQueueWorker = config.getInt(
                                                        "system(" + i + ").NumOfQueueWorker");
                                        ...
                                        if (nameSystem.toLowerCase().equals("transaction")) {
                                                transactionHost = config
                                                        .getString("system(" + i + ").host");
                                                transactionPort = config.getInt(
                                                        "system(" + i + ").port");
                                                GTXTransactionCacheMaxItem = config.getInt(
                                                        "system(" + i
                                                        + ").GTXTransactionCacheMaxItem");
                                                TransactionCommitLog = config.getString(
                                                        "system(" + i + ").TransactionCommitLog");
                                                TransactionCommitFile = config.getInt(
                                                        "system(" + i + ").TransactionCommitFile");
                                                T_TransactionCacheMaxItem = config.getInt(
                                                        "system(" + i
                                                        + ").T_TransactionCacheMaxItem");

                                                T_InvoiceCommitLog = config.getString(
                                                        "system(" + i + ").T_InvoiceCommitLog");
                                                T_InvoiceCommitFile = config.getInt(
                                                        "system(" + i + ").T_InvoiceCommitFile");
                                                T_InvoiceCacheMaxItem = config.getInt(
                                                        "system(" + i + ").T_InvoiceCacheMaxItem");

                                                T_AccountCommitLog = config.getString(
                                                        "system(" + i + ").T_AccountCommitLog");
                                                T_AccountCommitFile = config.getInt(
                                                        "system(" + i + ").T_AccountCommitFile");
                                                T_AccountCacheMaxItem = config.getInt(
                                                        "system(" + i + ").T_AccountCacheMaxItem");
                                        ...
                                        if (nameSystem.toLowerCase().equals("report")) {
                                                private_Key = config.getString(
                                                        "system(" + i + ").privatekey");
                                                dayReport = config.getString(
                                                        "system(" + i + ").dayReport");
                                                reporter = config.getString(
                                                        "system(" + i + ").reporter");
                                                sha1_shared_key = config.getString(
                                                        "system(" + i + ").sha1_shared_key");
...
//					LogUtil.info("Read information of system:" + nameSystem);
                        ...
                        LogUtil.error(
                                "Error to load property of file configuration : " + file
                                + ". And system is shutdown");
                        System.exit(1);
                }
        }
